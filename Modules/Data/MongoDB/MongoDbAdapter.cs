...
namespace Nyan.Modules.Data.MongoDB
{
    public class MongoDbAdapter : DataAdapterPrimitive
    {
            ...
            try
            {
                ...
                foreach (var prop in typeof(T).GetProperties())
                {
                    ...
                    if (pType.IsPrimitiveType())
                    {
                        ...
                        if ((pType.BaseType != null) && typeof(IList).IsAssignableFrom(pType.BaseType)
                            && pType.BaseType.IsGenericType) continue;
                        ...
                        if (MicroEntity<T>.Statements.PropertyFieldMap.ContainsKey(pSourceName)) pSourceName = MicroEntity<T>.Statements.PropertyFieldMap[pSourceName];
                ...
                if (MicroEntity<T>.Statements.IdColumn != null)
                {
                    ...
                    //Primary Key
                    var pkName = tn + "_PK";
                    var pkStat = "ALTER TABLE " + tn + " ADD (CONSTRAINT " + pkName + " PRIMARY KEY ("
                                 + MicroEntity<T>.Statements.IdColumn + "))";

                    try
                    {
                        Current.Log.Add(typeof(T).FullName + " : Adding Primary Key constraint " + pkName + " ("
                                        + MicroEntity<T>.Statements.IdColumn + ")");
                ...
                try
                {
                    Current.Log.Add(typeof(T).FullName + " : Adding BI Trigger "
                ...
                try
                {
                    Current.Log.Add(typeof(T).FullName + " : Adding BU Trigger "
        ...
        public override ModelDefinition GetModelfromStatements<T>(MicroEntityCompiledStatements refs,
            Definition.DdlContent scope)
        {
            ...
            if (doSchema)
            {
                ...
                foreach (var schemaElement in refs.SchemaElements)
                {
                    ...
                    try
                    {
                        ...
                        if (fetch.IndexOf("USING INDEX", StringComparison.Ordinal) != -1) fetch = fetch.Substring(0, fetch.IndexOf("USING INDEX", StringComparison.Ordinal)) + ")";

                        if (fetch.IndexOf("SEGMENT CREATION DEFERRED", StringComparison.Ordinal) != -1)
                            fetch = fetch.Substring(0,
                                fetch.IndexOf("SEGMENT CREATION DEFERRED", StringComparison.Ordinal));

                        if (fetch.IndexOf("ALTER TRIGGER", StringComparison.Ordinal) != -1) fetch = fetch.Substring(0, fetch.IndexOf("END;", StringComparison.Ordinal)) + "END";
                    ...
                    {
                        var le = "{2} - Exception while fetching {0} {1}: {3}".format(schemaElement.Value.Key,
                            schemaElement.Value.Value, refs.Label, e.Message);
            ...
            if (!doData) return ret;
            {
                ...
                foreach (var col in cols)
                {
                    ...
                    if (col["DATA_TYPE"].ToString().IndexOf("VARCHAR", StringComparison.Ordinal) != -1) delimiter = "'";

                    if (col["DATA_TYPE"].ToString().IndexOf("CLOB", StringComparison.Ordinal) != -1) delimiter = "'";
                ...
                try
                {
                        ...
                        foreach (var col in cols)
                        {
                            ...
                            else
                            {
                                if (valType.IndexOf("TIMESTAMP", StringComparison.Ordinal) != -1)
                                    valList += "TO_DATE('" + ((DateTime) val).ToString("dd/MM/yyyy HH:mm:ss")
                                               + "','DD/MM/YYYY HH24:MI:SS')";

                                else if (valType.IndexOf("DATE", StringComparison.Ordinal) != -1)
                                    valList += "TO_DATE('" + ((DateTime) val).ToString("dd/MM/yyyy 00:00:00")
                                               + "','DD/MM/YYYY HH24:MI:SS')";
                                else if (valType.IndexOf("CLOB", StringComparison.Ordinal) != -1)
                                {
                                    var splitpart = val.ToString().SplitInChunksUpTo(1800).ToList();

                                    for (var i = 0; i < splitpart.Count; i++) splitpart[i] = splitpart[i].Replace("'", "''").Replace("&", "&'||'");
                                ...
                                else valList += delim + val.ToString().Replace("'", "''").Replace("&", "&'||'") + delim;
                            }
                        }
                    ...
                    if (anyRows)
                    {
                        ...
                        data.AppendLine("TRUNCATE TABLE " + refs.SchemaElements["Table"].Value + ";"
                                        + Environment.NewLine);
                        data.AppendLine("ALTER TRIGGER \"" + refs.SchemaElements["BeforeInsertTrigger"].Value
                                        + "\" DISABLE;");
                        data.AppendLine("ALTER TRIGGER \"" + refs.SchemaElements["BeforeUpdateTrigger"].Value
                                        + "\" DISABLE;");
                        ...
                        if (canBuffer) data.AppendLine("SELECT * FROM dual;" + Environment.NewLine);
