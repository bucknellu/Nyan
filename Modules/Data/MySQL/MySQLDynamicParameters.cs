...
namespace Nyan.Modules.Data.MySql
{
    public class MySqlDynamicParameters : DynamicParametersPrimitive, SqlMapper.IDynamicParameters
    {
        ...
        public MySqlDynamicParameters(object template)
        {
        ...
        private void AddDynamicParams(object template)
        {
            ...
            var subDynamic = template as MySqlDynamicParameters;
            if (subDynamic == null)
            {
        ...
        public override void AddParameters(IDbCommand command, SqlMapper.Identity identity)
        {
            ...
            foreach (var param in Parameters)
            {
                ...
                var add = !((MySqlCommand)command).Parameters.Contains(name);

                MySqlParameter p;

                if (add)
                {
                    p = ((MySqlCommand)command).CreateParameter();
                    p.ParameterName = name;
                }
                else
                {
                    p = ((MySqlCommand)command).Parameters[name];
                }
