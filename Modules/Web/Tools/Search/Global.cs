    ...
    public static class Global
    {
        ...
        public static Dictionary<string, object> Run(string term, string categories)
        {
            ...
            try
            {
                ...
                var sortCon = new Dictionary<string, List<SearchResult>>();

                foreach (var i in retcon.ToList())
                {
                    sortCon[i.Key] = i.Value.Distinct(new SearchResult.Comparer()).OrderBy(j => !j.IsKeyMatch).ToList();
                }
        ...
        private static KeyValuePair<string, SearchResultBlock> ProcessSearchableType(ISearch instance, string term)
        {
            ...
            try { ret.Results = instance.SimpleQuery(term); } catch (Exception e) { Current.Log.Add(e); }
